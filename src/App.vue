  ...
  <div id="app-shell" class="min-h-screen bg-slate-950 text-slate-100">
    ...
    >
      <div id="app-layout" class="flex w-full items-start gap-6">
        <SidebarNav :nav-items="navItems" :active-item="currentView" @navigate="onNavigate" />

        <MainPanel>
          <component :is="activeView.component" v-if="activeView.component" @back="goHome" />
          <section v-else id="unavailable-view" class="max-w-xl">
            <span class="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300"
              >Coming soon</span
            >
