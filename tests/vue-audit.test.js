...
function collectContradictoryStateChecks(source) {
  ...
  for (const condition of checks) {
    const matches = [...condition.matchAll(/([A-Za-z_$][\w$]*)\s*(===|!==)\s*["']([^"']+)["']/g)];

    for (const match of matches) {
      const [, stateName, operator, literalValue] = match;
      const stateData = stateSignatures.get(stateName) ?? { equal: new Set(), notEqual: new Set() };
...
test("Vue files keep shared structure and style ownership consistent", () => {
  ...
  for (const filePath of vueFiles) {
    ...
    for (const structuralTag of ["main", "aside", "nav", "section"]) {
      for (const openingTag of collectOpeningTags(source, structuralTag)) {
        if (!/\bid\s*=/.test(openingTag)) {
          findings.push(`${relativePath} contains <${structuralTag}> without a stable id`);
        }
      }
    }
    ...
    for (const styleBlock of styleBlocks) {
      ...
      if (path.basename(filePath) !== "AdultView.vue") {
        findings.push(`${relativePath} owns component CSS outside the schedule exception`);
        continue;
      }
      ...
      if (selectors.some((selector) => !selector.startsWith("schedule-"))) {
        findings.push(`${relativePath} contains a non-schedule component CSS selector`);
      }
    }
  }
